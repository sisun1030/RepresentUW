...
if($task == "approve") {
  $place_id = htmlspecialchars($_GET['place_id']);
  $place_title = $_GET['place_title'];
  mysql_query("UPDATE places SET approved=1 WHERE id='$place_id'") or die(mysql_error());
  header("Location: index.php?view=$view&search=$search&p=$p");
  $message = "Welcome ".$place_title." to http://RepresentUW.com";
  tweet($message);
...
if($task == "delete") {
  ...
  mysql_query("DELETE FROM founder WHERE company='$place_id'") or die(mysql_error());
  header("Location: index.php?view=$view&search=$search&p=$p");
  exit;
}
          ...
          <li>
            <div class='options'>
              ...
              if($place[approved] == 1) {
                ...
                echo "
                  <a class='btn btn-small btn-success' href='index.php?task=approve&place_id=$place[id]&place_title=$place[title]&view=$view&search=$search&p=$p'>Approve</a>
